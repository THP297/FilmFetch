...
function PUSH_related(id, related_movie) {
    ...
    function get_related_movie_url(id) {
        ...
        let url = MOVIES_DETAIL_URL + String(id) + "/similar" + API_KEY + random_page;
        return url;
    }
...
function watch_movie(element) {
    ...
    fetch('movie_detail.html').then(res => res.text())
        .then(html => {
            ...
            fetch(url).then(res => res.json())
                .then(data => {
                    const MOVIE_VIDEO_URL = MOVIES_DETAIL_URL + String(element.id) + "/videos" + API_KEY;
                    ...
                    const list_of_data = [
                        data.original_title,
                        data.adult, data.vote_average,
                        data.release_date, data.production_countries[0]['name'],
                        data.spoken_languages[0]['name'],
                        data.genres[0]['name'],
                        companies,
                    ]
                    const data_desc = ['Movie Name:', 'Adult:', 'Rating:', 'Release Date:', 'Country:', 'Language:', 'Genres:', 'Companies:']
...
export function create_movie_frame(element) {
    ...
    vote.setAttribute("style", "color: yellowgreen");


    const overview_button = document.createElement('button')
    overview_button.innerHTML = "Watch";
    ...
    movieCol.classList.add("col-2", "col-xl-2", "col-lg-3", "col-md-3", "col-sm-4", "col-xs-12","movie-frame");
    ...
    movieCol.appendChild(moviePoster);
    movieCol.appendChild(movieTitle);
    movieCol.appendChild(overview_frame);
